/****************************************************************************
 * boards/arm/CONFIG_DEBUG_LEDS=yf4/at32f450zk-eval/src/at32f4xx_appinit.c
 *
...
#ifdef CONFIG_AT32F4_ROMFS
#include "at32f4xx_romfs.h"
#endif

#include "at32f450z_eval.h"
  ...
  if (!initialized)
    {
      ...
      ret = at32_romfs_initialize();
      if (ret < 0)
        {
          syslog(LOG_ERR, "ERROR: Failed to mount romfs at %s: %d\n",
                CONFIG_AT32F4_ROMFS_MOUNTPOINT, ret);
        }
#endif

#ifdef CONFIG_FS_NXFFS

#  ifdef CONFIG_AT32F4_PROGMEM

      /* Create an instance of the AT32F4 FLASH program memory
       * device driver
       */
      ...
      ret = nx_mount(NULL, CONFIG_AT32F4_NXFFS_MOUNTPT, "nxffs", 0, NULL);
      if (ret < 0)
        {
...
#  ifdef HAVE_AT24
     ...
     ret = at32_at24_wr_test(AT24_MINOR);
      if (ret < 0)
        {
